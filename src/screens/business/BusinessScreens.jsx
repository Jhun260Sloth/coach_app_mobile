...
export function ScreenBusinessBookingDetail() {
  ...
  const cancel = () => { updateBusinessBooking(booking.id, { status: "cancelled", paymentStatus: paid ? "refund_processing" : "not_requested", refundStatus: paid ? "processing" : null }); pushNotification({ audience: "client", type: "booking", title: "Business booking cancelled", body: `${business.tradingName} cancelled ${program?.title || booking.service}. ${paid ? "Your refund is being processed." : "No payment was collected."}`, bookingId: booking.id }); if (booking.assignedCoachId) pushNotification({ audience: "businessCoach", businessId: business.id, coachId: booking.assignedCoachId, type: "booking", title: "Assigned session cancelled", body: `${program?.title || booking.service} with ${booking.clientName} was cancelled by the business.`, bookingId: booking.id }); setCancelOpen(false); toast("Booking cancelled"); nav("business-bookings"); };
  const reschedule = () => { if (!scheduleDraft.date.trim() || !scheduleDraft.time.trim()) { toast("Add a date and time"); return; } updateBusinessBooking(booking.id, scheduleDraft); pushNotification({ audience: "client", type: "booking", title: "Business booking rescheduled", body: `${program?.title || booking.service} moved to ${scheduleDraft.date} at ${scheduleDraft.time}.`, bookingId: booking.id }); if (booking.assignedCoachId) pushNotification({ audience: "businessCoach", businessId: business.id, coachId: booking.assignedCoachId, type: "booking", title: "Assigned session rescheduled", body: `${program?.title || booking.service} moved to ${scheduleDraft.date} at ${scheduleDraft.time}.`, bookingId: booking.id }); setRescheduleOpen(false); toast("Booking rescheduled"); };
  ...
  const reportIssue = () => nav("dispute-create", { bookingId: booking.id, role: "business", category: "client_no_show", backTo: "business-booking-detail", backParams: { id: booking.id } });
  return <div style={page(C)}><TopBar title="Booking details" onBack={() => nav("business-bookings")} right={<StatusPill status={booking.status} perspective="business" />} />
    <div style={{ ...scroll, paddingTop: 10, paddingBottom: 32 }} className="cl-hide-scrollbar">
      ...
      <div style={{ display: "grid", gap: 9, marginTop: 20 }}>
        ...
        {booking.status === "confirmed" ? <Btn full icon={PlayCircle} onClick={() => nav("coach-session-start", { bookingId: booking.id, role: "business", backTo: "business-booking-detail", backParams: { id: booking.id } })}>Open session check-in</Btn> : null}
        {booking.status === "in_progress" ? <><Btn full icon={PlayCircle} onClick={() => nav("session-progress", { bookingId: booking.id, role: "business", backTo: "business-booking-detail", backParams: { id: booking.id } })}>View live session</Btn><Btn full variant="outline" onClick={() => nav("session-completion", { bookingId: booking.id, role: "business", backTo: "business-booking-detail", backParams: { id: booking.id } })}>Finish session</Btn></> : null}
        {booking.status === "completion_pending" ? <Btn full icon={finalCharge ? CreditCard : CheckCircle2} onClick={() => nav("session-completion", { bookingId: booking.id, role: "business", backTo: "business-booking-detail", backParams: { id: booking.id } })}>{finalCharge ? "Review final payment" : "Finish session"}</Btn> : null}
        {booking.status === "completed" ? <Btn full icon={Banknote} onClick={() => nav("business-earnings")}>View payment settlement</Btn> : null}
  ...
  return <div style={page(C)}><TopBar title="Finance & billing" onBack={() => nav("business-more")} />
    <div style={{ ...scroll, paddingBottom: 32 }} className="cl-hide-scrollbar">
      ...
      <Card style={{ padding: 14, marginTop: 10 }}>
        ...
        {business.payoutMethod ? <div style={{ marginTop: 14, paddingTop: 12, borderTop: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 9 }}><Landmark size={17} color={C.brand} /><div style={{ flex: 1, fontSize: T.captionLg, color: C.slate, ...fBody }}>{business.payoutMethod.bank} · •••• {business.payoutMethod.accountLast4}</div></div> : null}
...
export function ScreenBusinessIdentityDetails() {
  const { darkMode, nav, business } = useApp(); const C = darkMode ? CD : CL;
  return <div style={page(C)}><TopBar title="Business identity" subtitle="Registration & account representative" onBack={() => nav("business-compliance")} /><div style={{ ...scroll, paddingBottom: 32 }} className="cl-hide-scrollbar"><Card style={{ padding: 16, marginTop: 10, background: C.successTint }}><div style={{ display: "flex", gap: 11, alignItems: "center" }}><ShieldCheck size={24} color={C.success} /><div><div style={{ fontSize: T.bodyLg, fontWeight: 700, color: C.jet, ...fDisplay }}>Identity verified</div><div style={{ marginTop: 3, fontSize: T.captionLg, color: C.slate, ...fBody }}>Registration and representative details have passed review.</div></div></div></Card><SectionLabel style={{ marginTop: 24 }}>Registered business</SectionLabel><Card style={{ marginTop: 10, padding: "0 14px" }}><Row label="Legal name" value={business.legalName} /><Row label="Trading name" value={business.tradingName} /><Row label="Entity type" value={business.entityType} /><Row label="ABN" value={business.abn} /><Row label="GST registered" value={business.gstRegistered ? "Yes" : "No"} /><Row label="Registered address" value={business.registeredAddress} last /></Card><SectionLabel style={{ marginTop: 24 }}>Account representative</SectionLabel><Card style={{ marginTop: 10, padding: "0 14px" }}><Row label="Name" value={business.ownerName} /><Row label="Role" value={business.representativeRole} /><Row label="Email" value={business.ownerEmail} last /></Card></div></div>;
}

export function ScreenBusinessInsuranceDetails() {
  const { darkMode, nav, business } = useApp(); const C = darkMode ? CD : CL;
  const insurance = business.insurance || {};
  return <div style={page(C)}><TopBar title="Insurance & safeguarding" subtitle="Coverage and participant safety" onBack={() => nav("business-compliance")} /><div style={{ ...scroll, paddingBottom: 32 }} className="cl-hide-scrollbar"><Card style={{ padding: 16, marginTop: 10, background: insurance.valid ? C.successTint : C.warnTint }}><div style={{ display: "flex", gap: 11, alignItems: "center" }}><FileCheck2 size={24} color={insurance.valid ? C.success : C.warnStrong} /><div><div style={{ fontSize: T.bodyLg, fontWeight: 700, color: C.jet, ...fDisplay }}>{insurance.valid ? "Coverage is current" : "Coverage needs attention"}</div><div style={{ marginTop: 3, fontSize: T.captionLg, color: C.slate, ...fBody }}>Keep these details current to remain discoverable and accept bookings.</div></div></div></Card><SectionLabel style={{ marginTop: 24 }}>Public liability insurance</SectionLabel><Card style={{ marginTop: 10, padding: "0 14px" }}><Row label="Insurer" value={insurance.insurer || "Not provided"} /><Row label="Policy number" value={insurance.policyNumber || "Not provided"} /><Row label="Coverage" value={insurance.coverage || "Not provided"} /><Row label="Expiry" value={insurance.expiry || "Not provided"} /><Row label="Status" value={insurance.valid ? "Verified" : "Review required"} last /></Card><SectionLabel style={{ marginTop: 24 }}>Safeguarding</SectionLabel><Card style={{ marginTop: 10, padding: 14, display: "flex", gap: 11, alignItems: "flex-start" }}><ShieldCheck size={20} color={insurance.safeguardingAccepted ? C.success : C.warnStrong} /><div><div style={{ fontSize: T.body, fontWeight: 700, color: C.jet, ...fBody }}>{insurance.safeguardingAccepted ? "Safeguarding standards accepted" : "Safeguarding acceptance required"}</div><div style={{ marginTop: 4, fontSize: T.captionLg, lineHeight: 1.5, color: C.slate, ...fBody }}>Business admins remain responsible for eligible coaches, guardian consent and safe program delivery.</div></div></Card></div></div>;
}
