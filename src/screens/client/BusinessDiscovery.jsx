    ...
    <Card onClick={onOpen} ariaLabel={`View ${business.tradingName}`} style={{ padding: 0, marginBottom: 12, overflow: "hidden", ...style }}>
      ...
      <div style={{ padding: 14 }}>
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          <div style={{ width: 54, height: 54, borderRadius: 14, background: C.white, padding: 4, boxShadow: "0 2px 6px rgba(0,0,0,0.06)", border: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", gap: 8, marginTop: 12, paddingTop: 10, borderTop: `1px solid ${C.border}`, fontSize: T.captionLg, color: C.slate, ...fBody }}>
          ...
          <span>{rosterCount} coach{rosterCount === 1 ? "" : "es"} · {programCount} program{programCount === 1 ? "" : "s"}</span>
        </div>
      </div>
    </Card>
  );
    ...
    <div style={{ ...page(C), position: "relative" }}>
        ...
        <div style={{ position: "relative", height: 175, background: bannerGradient, overflow: "hidden", flexShrink: 0 }}>
          ...
          <div style={{ position: "absolute", top: 12, left: 16, right: 16, display: "flex", justifyContent: "space-between", alignItems: "center", zIndex: 10 }}>
            ...
            <button
              ...
              style={{ width: 40, height: 40, borderRadius: 999, background: "rgba(18,20,24,0.40)", backdropFilter: "blur(10px)", border: "1px solid rgba(255,255,255,0.22)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}
            >
              <MessageCircle size={19} color={C.white} />
            </button>
          </div>
        </div>
      ...
      <div style={{ padding: "0 18px" }}>
        ...
        {tab === "about" && <div style={{ marginTop: 16 }}>
          ...
          {roster.length ? <><div style={{ marginTop: 22, fontSize: T.title, fontWeight: 700, color: C.jet, ...fDisplay }}>Meet the coaching team</div><div className="cl-swipe-row" style={{ display: "flex", gap: 14, overflowX: "auto", margin: "12px -18px 0", padding: "0 18px 8px" }}>{roster.map((coach) => <button type="button" key={coach.id} aria-label={`View ${coach.name}`} onClick={() => nav("business-coach-public-profile", { businessId: business.id, coachId: coach.id })} style={{ width: 82, minHeight: 98, flexShrink: 0, textAlign: "center", border: "none", borderRadius: 16, padding: "8px 3px", background: C.fog, cursor: "pointer" }}><Avatar name={coach.name} size={54} /><span style={{ display: "block", marginTop: 7, fontSize: T.captionLg, fontWeight: 700, color: C.jet, ...oneLine, ...fBody }}>{coach.name.split(" ")[0]}</span><span style={{ display: "block", marginTop: 2, fontSize: T.micro, color: C.slate, ...fBody }}>{coach.sport}</span></button>)}</div></> : null}
...
export function ScreenBusinessCoachPublicProfile() {
  const { darkMode, nav, params, businesses, businessRoster, businessPrograms } = useApp();
  const C = darkMode ? CD : CL;
  const business = businesses.find((item) => item.id === params?.businessId);
  const coach = businessRoster.find((item) => item.id === params?.coachId && item.businessId === business?.id);
  const programs = businessPrograms.filter((item) => coach?.programs?.includes(item.id) && item.status === "live");
  if (!business || !coach) return <div style={page(C)}><TopBar title="Coach profile" onBack={() => nav("business-directory")} /><EmptyState icon={User} title="Coach unavailable" body="This coach may no longer be on the organisation’s active roster." /></div>;
  return <div style={page(C)}>
    <TopBar title="Coach profile" onBack={() => nav("business-public-profile", { id: business.id, tab: "about" })} />
    <div style={{ ...scroll, paddingTop: 20 }} className="cl-hide-scrollbar">
      <div style={{ textAlign: "center" }}><Avatar name={coach.name} size={86} ring /><div style={{ marginTop: 12, fontSize: T.display, fontWeight: 800, color: C.jet, ...fDisplay }}>{coach.name}</div><div style={{ marginTop: 5, fontSize: T.body, color: C.slate, ...fBody }}>{coach.sport} coach</div><div style={{ display: "flex", justifyContent: "center", gap: 6, flexWrap: "wrap", marginTop: 10 }}><Badge tone="success" icon={ShieldCheck}>Identity verified</Badge>{coach.wwcc === "current" ? <Badge tone="success" icon={BadgeCheck}>WWCC current</Badge> : null}</div></div>
      <Card onClick={() => nav("business-public-profile", { id: business.id })} ariaLabel={`Open ${business.tradingName}`} style={{ padding: 14, marginTop: 22, display: "flex", alignItems: "center", gap: 12 }}><div style={{ width: 46, height: 46, borderRadius: 13, padding: 4, border: `1px solid ${C.border}`, background: C.white, overflow: "hidden" }}><img src={business.profile.logo} alt={`${business.tradingName} logo`} style={{ width: "100%", height: "100%", objectFit: "contain" }} /></div><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: T.bodyLg, fontWeight: 700, color: C.jet, ...fDisplay }}>{business.tradingName}</div><div style={{ marginTop: 3, fontSize: T.captionLg, color: C.slate, ...fBody }}>Bookings and payments managed by the organisation</div></div><ChevronRight size={17} color={C.slateLight} /></Card>
      <Card style={{ padding: 14, marginTop: 12, background: C.brandTint, display: "flex", gap: 10 }}><ShieldCheck size={19} color={C.brand} /><div style={{ fontSize: T.captionLg, color: C.slate, lineHeight: 1.55, ...fBody }}>CoachNivo verifies this coach separately from the business. Programs, availability, customer support and payments remain owned by {business.tradingName}.</div></Card>
      <div style={{ marginTop: 24, fontSize: T.title, fontWeight: 700, color: C.jet, ...fDisplay }}>Programs with {coach.name.split(" ")[0]}</div>
      <div style={{ marginTop: 10 }}>{programs.length ? programs.map((program) => <Card key={program.id} onClick={() => nav("business-program-detail", { businessId: business.id, programId: program.id })} ariaLabel={`View ${program.title}`} style={{ padding: 14, marginBottom: 10, display: "flex", alignItems: "center", gap: 11 }}><div style={{ width: 40, height: 40, borderRadius: 12, background: C.brandTint, display: "flex", alignItems: "center", justifyContent: "center" }}><CalendarDays size={18} color={C.brand} /></div><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: T.body, fontWeight: 700, color: C.jet, ...fDisplay }}>{program.title}</div><div style={{ marginTop: 3, fontSize: T.captionLg, color: C.slate, ...fBody }}>{program.type} · {program.durationMinutes} min</div></div><ChevronRight size={16} color={C.slateLight} /></Card>) : <EmptyState icon={CalendarDays} title="No current programs" body="View the organisation for other available coaching programs." />}</div>
      <Btn full icon={MessageCircle} onClick={() => nav("chat-thread", { name: business.tradingName, context: `Question about ${coach.name}`, backTo: "business-coach-public-profile", backParams: { businessId: business.id, coachId: coach.id } })}>Message the organisation</Btn>
    </div>
  </div>;
}
...
export function ScreenClientBusinessBookingDetail() {
  ...
  const cancel = () => { const wasPending = booking.status === "pending"; const wasPaid = ["held", "released"].includes(booking.paymentStatus); updateBusinessBooking(booking.id, { status: "cancelled", paymentStatus: wasPaid ? "refund_processing" : "not_requested", refundStatus: wasPaid ? "processing" : null }); pushNotification({ audience: "business", businessId: booking.businessId, type: "booking", title: wasPending ? "Booking request withdrawn" : "Booking cancelled", body: `${booking.clientName || "A client"} ${wasPending ? "withdrew their request for" : "cancelled"} ${program?.title || booking.service}.${wasPaid ? " A refund is processing." : ""}`, bookingId: booking.id }); if (booking.assignedCoachId) pushNotification({ audience: "businessCoach", businessId: booking.businessId, coachId: booking.assignedCoachId, type: "booking", title: "Assigned session cancelled", body: `${program?.title || booking.service} with ${booking.clientName || "the client"} was cancelled.`, bookingId: booking.id }); toast(wasPending ? "Booking request withdrawn" : wasPaid ? "Booking cancelled — refund processing" : "Booking cancelled"); nav("client-dashboard"); };
  const pay = () => nav("business-booking-payment", { bookingId: booking.id });
  ...
  return <div style={page(C)}><TopBar title="Booking details" onBack={() => goBack("client-dashboard")} /><div style={{ ...scroll, paddingTop: 20, paddingBottom: 32 }} className="cl-hide-scrollbar">
    ...
    <Card style={{ padding: 16, marginTop: 12 }}><div style={{ display: "grid", gap: 11, fontSize: T.body, color: C.slate, ...fBody }}><div>Date & time <strong style={{ color: C.jet }}>{booking.date} · {booking.time}</strong></div><div>Participant <strong style={{ color: C.jet }}>{booking.participants}</strong></div><div>Coach <strong style={{ color: C.jet }}>{coach?.name || booking.coachName || "To be assigned"}</strong></div><div>Format <strong style={{ color: C.jet }}>{booking.mode}</strong></div></div>{coach ? <div style={{ marginTop: 12, paddingTop: 10, borderTop: `1px solid ${C.border}` }}><Btn full size="sm" variant="outline" onClick={() => nav("business-coach-public-profile", { businessId: business.id, coachId: coach.id })}>View coach profile</Btn></div> : null}</Card>
    <div style={{ marginTop: 12 }}><BusinessBookingPaymentSummary booking={booking} C={C} perspective="client" /></div>
