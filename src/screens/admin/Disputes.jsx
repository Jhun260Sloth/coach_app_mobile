  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      <div style={{ padding: "18px 20px 0" }}>
        <div style={{ fontSize: 22, fontWeight: 600, color: C.jet, ...fDisplay }}>Disputes</div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", paddingBottom: 20 }}>
        <Card style={{ marginBottom: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              <div style={{ fontSize: 14, fontWeight: 600, color: C.jet, ...fDisplay }}>{dispute.issue}</div>
              <div style={{ fontSize: 12, color: C.slate, marginTop: 2, ...fBody }}>{dispute.parties}</div>
            </div>
            <Badge tone="orange">Open</Badge>
          </div>
          <p style={{ fontSize: 12.5, color: C.slate, lineHeight: 1.6, marginTop: 10, ...fBody }}>{dispute.summary}</p>
        </Card>
        ...
        {dispute.evidence.map((e, i) => (
          <Card key={i} style={{ marginBottom: 10, display: "flex", gap: 12, alignItems: "center" }}>
            ...
            <div>
              <div style={{ fontSize: 12.5, fontWeight: 600, color: C.jet, ...fBody }}>{e.type}</div>
              <div style={{ fontSize: 11.5, color: C.slate, marginTop: 2, ...fBody }}>{e.label}</div>
            </div>
          </Card>
        ))}
        ...
        <Card style={{ marginBottom: 14 }}>
          {dispute.messages.map((m, i) => (
            <div key={i} style={{ marginBottom: i === dispute.messages.length - 1 ? 0 : 12, paddingBottom: i === dispute.messages.length - 1 ? 0 : 12, borderBottom: i === dispute.messages.length - 1 ? "none" : `1px solid ${C.border}` }}>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span style={{ fontSize: 12.5, fontWeight: 600, color: C.jet, ...fBody }}>{m.from}</span>
                <span style={{ fontSize: 11, color: C.slateLight, ...fBody }}>{m.time}</span>
              </div>
              <p style={{ fontSize: 12.5, color: C.slate, marginTop: 4, lineHeight: 1.55, ...fBody }}>{m.text}</p>
            </div>
          ))}
        </Card>
      </div>
