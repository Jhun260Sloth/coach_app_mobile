  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      <div style={{ padding: "18px 20px 0" }}>
        <div style={{ fontSize: T.display, fontWeight: 600, color: C.jet, ...fDisplay }}>Verification queue</div>
      </div>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <div style={{ fontSize: T.bodyLg, fontWeight: 600, color: C.jet, ...fDisplay }}>{v.name}</div>
                  {v.submittedByUser && <Badge tone="orange">New</Badge>}
                </div>
                <div style={{ fontSize: T.label, color: C.slate, ...fBody }}>{v.sport} · Submitted: {v.type}</div>
              </div>
    ...
    <div style={{ padding: "20px 20px 0", height: "100%", display: "flex", flexDirection: "column", position: "relative" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", paddingBottom: 20 }}>
        <Card style={{ marginBottom: 14, display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            <div style={{ fontSize: T.subtitleLg, fontWeight: 600, color: C.jet, ...fDisplay }}>{applicant.name}</div>
            <div style={{ fontSize: T.label, color: C.slate, ...fBody }}>{applicant.sport} · {applicant.suburb}</div>
            <div style={{ fontSize: T.captionLg, color: C.slate, ...fBody }}>{applicant.experience}</div>
          </div>
        </Card>
        ...
        {applicant.documents.map((doc) => (
          <Card key={doc.key} onClick={() => setViewingDoc(doc)} style={{ marginBottom: 10, display: "flex", gap: 12, alignItems: "center" }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: T.captionLg, color: C.slate, marginTop: 2, ...fBody }}>{doc.detail}</div>
            </div>
            <Badge tone="success" icon={CheckCircle2}>Uploaded</Badge>
            <ChevronRight size={15} color={C.slateLight} />
          </Card>
        ))}
        <div style={{ fontSize: T.caption, color: C.slateLight, marginTop: -4, marginBottom: 10, ...fBody }}>Tap a document to view it.</div>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", gap: 8, background: C.fog, borderRadius: 12, padding: 12, marginTop: 6 }}>
          ...
          <span style={{ fontSize: T.label, color: C.slate, lineHeight: 1.5, ...fBody }}>Confirm each document is valid and matches the applicant's stated sport and coaching claims before approving.</span>
        </div>
      </div>
        ...
        <div style={{ position: "absolute", inset: 0, background: "rgba(22,24,29,.7)", display: "flex", alignItems: "flex-end", zIndex: 80 }}>
          <div style={{ width: "100%", background: C.white, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 20, maxHeight: "82%", display: "flex", flexDirection: "column" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
              <div style={{ fontSize: T.subtitleLg, fontWeight: 600, color: C.jet, ...fDisplay }}>{viewingDoc.label}</div>
            ...
            <div style={{
              ...
              <span style={{ fontSize: T.labelLg, color: "rgba(255,255,255,.7)", ...fBody }}>Document preview</span>
            </div>
            <div style={{ fontSize: T.labelLg, color: C.slate, lineHeight: 1.55, marginBottom: 16, ...fBody }}>{viewingDoc.detail}</div>
